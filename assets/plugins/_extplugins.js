...
const QueenJulirt = require('queenjuliet-public');
...
Juliet.operate({pattern: 'install ?(.*)', fromMe: true, dontAddCommandList: true, deleteCommand: false,}, (async (message, match) => {
    await QueenJuliet.juliet_setup()
    if (match[1] === '') return await message.sendMessage(Lang.NEED_URL + '.install https://gist.github.com/Endpanda/49afd28b1932095cc76facbcedef3482')
    try {
...
Juliet.operate({pattern: 'plugin', fromMe: true, deleteCommand: false, dontAddCommandList: true}, (async (message, match) => {
    await QueenJuliet.juliet_setup()
...
Juliet.operate({pattern: 'remove(?: |$)(.*)', fromMe: true, dontAddCommandList: true}, (async (message, match) => {
    await QueenJuliet.juliet_setup()
